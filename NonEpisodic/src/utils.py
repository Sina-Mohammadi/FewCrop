...
def make_log_dir(log_path, method):
    log_dir = os.path.join(log_path, method)
    if not os.path.exists(log_dir):
        os.makedirs(log_dir, exist_ok=True)
    return log_dir

def get_log_file(log_path, method):
    log_dir = make_log_dir(log_path=log_path, method=method)
    i = 0
...
def extract_mean_features(model, train_loader, args, logger, device):
    ...
    if args.PretrainedOnSource==False:
        filepath_mean = os.path.join(save_dir, 'output_mean_NotPretrainedOnSource.plk')
    else:  
    ...
    if not os.path.isfile(filepath_mean):
        ...
        with torch.no_grad():
            ...
            save_pickle(filepath_mean, [out_mean, fc_out_mean])
            return  torch.from_numpy(out_mean),  torch.from_numpy(fc_out_mean)
    else:
        out_mean, fc_out_mean = load_pickle(filepath_mean)
